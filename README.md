...
```
--------------------
# Application Binary Interface (ABI) - from wikipedia
* An interface exposed by software that is defined for in-process machine code access
* Relatively low level of abstraction
* Depends on the target hardware and software build toolchain
* Contrast with APIs which are relatively high level, hardware-independant and human-readable
* API defines source code level interface - ABI defines interface to compiled code

### Description - ABI Includes:
* Processor Instruction set
	* Register file structure
	* Memory access types
	* etc
* Size, layout, and alignment of basic data types that the processor can access
* Calling Convention - see below
* Name mangling
* Exception Propogation
* How to make system calls
* Binary format of object files, program libraries etc
* Example ABIs:
	* Intel Binary Compatibility Standard iBCS
	* System V Release 4

### Embedded ABI
* EABI used on an embedded operating system specifies aspects such as
	* File formats
	* Data types
	* Register usage
	* Stack frame organization
	* Function Parameter Passing
* Each compiler and assembler that support and EABI creates code that is compatible with code generated by other such compilers and assemblers.
* Typically optimized for performance and for the target system's limited resources:
	* May omit abstractions between kernel and user space found in desktop systems
	* e.g. dynamic linking avoided to allow smaller executables and faster loads
	* Fixed register usage allows more compact stacks and kernel calls
	* Running the application in privileged mode:
...
* Also, the many incompatible conventions of x86 has been reduced to two:
	* Microsoft x64 Calling Convention
		* Used in Windows and pre-boot UEFI (Unified Extensible Firmware Interface)
	* System V AMD64 ABI
...
| Wider Float | YMM + ZMM if available |

* For Leaf-node functions (functions which don't call other functions)
	* A 128-byte space is reserved beneath the stack pointer called the ***red zone***.
