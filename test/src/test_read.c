...
/*   Updated: 2025/10/27 15:00:58 by jcummins         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libasm.h"

int	test_read_n_fd(int fd, char *buf, ssize_t (*read_fn)(int fd, void *buf,
			size_t count), int n)
{
	int	bytes_read;
	int err;
...
int	test_read_n_func(const char *filename, char *buf, ssize_t (*read_fn)(int fd,
			void *buf, size_t count), int n)
{
	int	fd;
	...
	/*{*/
		...
		/*return (FAIL_READ);*/
	/*}*/
	test_read_n_fd(fd, buf, read_fn, n);
	return (SUCCESS);
}
...
int	test_read_n(const char *filename, int n)
{
	...
	printf("Testing ft_read against read from unistd.h with file '%s'...\n",
		filename);
	test_read_n_func(filename, ft_buf, ft_read, n);
	test_read_n_func(filename, std_buf, read, n);
	result = strcmp(std_buf, ft_buf);
	set_result_color(result);
	printf("\tunistd read: %s\n", std_buf);
	printf("\tft_read: %s\n", ft_buf);
